    ...
    public async findAllWorkspaceAndInstances(offset: number, limit: number, orderBy: keyof WorkspaceAndInstance, orderDir: "ASC" | "DESC", query?: AdminGetWorkspacesQuery, searchTerm?: string): Promise<{ total: number, rows: WorkspaceAndInstance[] }> {
        ...
        if (query) {
            ...
            if (query.instanceIdOrWorkspaceId) {
                ...
                // in additiong to adding "instanceId" to the "WHERE" clause like for the other workspace-guided queries, we need to modify the JOIN condition below
        ...
        // We need to select the latest wsi for a workspace. It's the same problem we have in 'find' (above), so we use the same approach.
        // Only twist is that we might be searching for an instance directly
        const workspaceRepo = await this.getWorkspaceRepo();
